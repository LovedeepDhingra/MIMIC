...
CREATE OR REPLACE TABLE daring-tracer-300615.mimiciv_full_cdm_april26.d_items_to_concept AS
WITH
counts AS
(
    ...
    SELECT
        ...
        COUNT(*)                    AS row_count
    FROM
        daring-tracer-300615.mimiciv_full_cdm_april26.lk_procedure_mapped src
    WHERE
...
CREATE OR REPLACE TABLE daring-tracer-300615.mimiciv_full_cdm_april26.d_labitems_to_concept AS
WITH
counts AS
(
    SELECT
    ...
    FROM
        daring-tracer-300615.mimiciv_full_cdm_april26.lk_meas_labevents_mapped
    GROUP BY itemid
)
SELECT
    -- itemid
...
FROM
    daring-tracer-300615.mimiciv_full_cdm_april26.lk_meas_d_labitems_concept src
LEFT JOIN
    counts 
        USING (itemid)
ORDER BY
...
CREATE OR REPLACE TABLE daring-tracer-300615.mimiciv_full_cdm_april26.d_micro_to_concept AS
WITH
counts AS
(
    SELECT
    ...
    FROM
        daring-tracer-300615.mimiciv_full_cdm_april26.lk_specimen_mapped
    GROUP BY itemid
    UNION ALL -- more unions
    SELECT
        test_itemid AS itemid, COUNT(*) AS row_count
    FROM
        daring-tracer-300615.mimiciv_full_cdm_april26.lk_meas_organism_mapped
    GROUP BY itemid
    UNION ALL -- more unions
    SELECT
    ...
    FROM
        daring-tracer-300615.mimiciv_full_cdm_april26.lk_meas_ab_mapped
    GROUP BY itemid
)
SELECT
    -- itemid
    ...
    counts.row_count                AS row_count
FROM
    daring-tracer-300615.mimiciv_full_cdm_april26.lk_d_micro_concept src
LEFT JOIN
    counts 
        USING (itemid)
LEFT JOIN
